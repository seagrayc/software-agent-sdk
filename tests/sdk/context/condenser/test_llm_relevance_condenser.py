...
def test_redacts_observation_in_place() -> None:
    ...
    # Build events and compute message index for the observation
    events = [
    ...
    for i, m in enumerate(messages):
        if m.role == "tool" and m.tool_call_id == tool_call_id:
            tool_call_index = i
            break
    assert tool_call_index is not None
    ...
    events_with_directive = events + [directive]
    view = View.from_events(events_with_directive)
    condenser = LLMRelevanceCondenser()
...
def test_condense_handles_duplicate_directives_once() -> None:
    ...
    observation = _observation(tool_call_id)
    # Build events and compute message index for the observation
    events = [_message("user request"), action, observation]
    view = View.from_events(events)
    ...
    events_with_directives = events + [directive_one, directive_two]
    view = View.from_events(events_with_directives)
    condenser = LLMRelevanceCondenser()
    redacted_view = condenser.condense(view)
    ...
    for ev in redacted_view.events:
        if (
            isinstance(ev, AgentErrorEvent)
            and ev.tool_call_id == observation.tool_call_id
        ):
