...
5. **Testing strategy**
   - Unit tests for schema validation (bad IDs, missing summary) and executor behaviour (duplicate IDs, unknown events) in `tests/sdk/tool/test_llm_relevance_condenser_tool.py`.
   - Unit tests for condenser application, including overlapping directives and missing events, in `tests/sdk/context/condenser/test_llm_relevance_condenser.py`.
   - Integration test using a scripted conversation to ensure: tool invocation adds directive, condenser emits condensation on next step, and the resulting view excludes forgotten events while preserving chain continuity.
   - Regression test for agent loop confirming that relevance condensing directives are indempotent - in the sense that multiple converstations steps will "re-process" the reductions on each step.
